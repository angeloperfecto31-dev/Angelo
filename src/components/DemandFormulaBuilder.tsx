...
export default function DemandFormulaBuilder({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden text-slate-800 dark:text-slate-100 max-w-5xl w-full max-h-[92vh] sm:max-h-[90vh] flex flex-col transition-all">
      {/* Header - Sticky Top */}
      <div className="shrink-0 p-4 sm:p-5 sm:pb-4 bg-slate-50/90 dark:bg-slate-950/90 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-start sm:items-center justify-between w-full sm:w-auto">
          <div className="flex items-center gap-3">
            <div className="p-2 sm:p-2.5 bg-indigo-600 dark:bg-indigo-500 rounded-xl sm:rounded-2xl text-white shadow-md shadow-indigo-500/20 shrink-0">
            ...
            <div>
              <div className="flex items-center flex-wrap gap-1.5 sm:gap-2">
                ...
                <span className="text-[9px] sm:text-[10px] font-black uppercase px-2 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                  PEC Sizing Engine
                </span>
              </div>
          ...
          {/* Close button on top right for mobile/desktop */}
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="sm:hidden p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors shrink-0"
        ...
        {/* Mode Selector + Close button for Desktop */}
        <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto">
          <div className="grid grid-cols-2 sm:flex items-center bg-slate-200/80 dark:bg-slate-800 p-1 rounded-xl sm:rounded-2xl border border-slate-300/60 dark:border-slate-700/60 w-full sm:w-auto">
            <button
              ...
              className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg sm:rounded-xl text-xs font-bold transition-all ${
                mode === "default"
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="hidden sm:flex p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors shrink-0"
              aria-label="Close dialog"
            >
              <X className="w-5 h-5" />
            </button>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-6 space-y-5 sm:space-y-6 overscroll-contain">
        {/* System Phase Tabs */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3 gap-2 sm:gap-4">
          ...
          <div className="flex items-center justify-between sm:justify-end gap-1.5 text-xs text-slate-400">
            <span className="flex items-center gap-1">
              <Sliders className="w-3.5 h-3.5 text-indigo-500" />
              <span>Editing: <strong className="text-slate-700 dark:text-slate-200 font-bold">{activeTab === "3PH" ? "3-Phase (3Φ)" : "Single-Phase (1Φ)"}</strong></span>
            </span>
        ...
        {mode === "default" && (
          <div className="p-3.5 sm:p-4 bg-blue-50/70 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/40 rounded-2xl flex flex-col sm:flex-row items-start justify-between gap-3 text-blue-800 dark:text-blue-300 text-xs">
            <div className="flex items-start gap-2.5">
              <HelpCircle className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-bold">Standard PEC 80% Default Calculation is Active</p>
            ...
            <button
              ...
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs self-end sm:self-center shrink-0"
            >
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400">Click to apply equation</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {currentPresets.map((preset) => (
              <button
                ...
                className="p-2.5 text-left bg-slate-50 hover:bg-indigo-50/70 dark:bg-slate-800/60 dark:hover:bg-indigo-950/40 border border-slate-200 hover:border-indigo-300 dark:border-slate-700 dark:hover:border-indigo-800 rounded-xl transition-all group active:scale-[0.98] cursor-pointer"
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <Layers className="w-3 h-3 text-slate-400 group-hover:text-indigo-500" />
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            <textarea
              ...
              className={`w-full p-3 sm:p-4 font-mono text-xs sm:text-sm md:text-base rounded-2xl border transition-all resize-none focus:outline-none focus:ring-2 ${
                mode === "default"
          ...
          >
              ...
              <div className="flex flex-wrap gap-2">
                {availableVariables.map((v) => (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/60 dark:hover:bg-indigo-900/80 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-xl text-xs font-bold transition-all shadow-xs active:scale-95 cursor-pointer min-h-[38px]"
                  >
                    <span className="font-mono">{v.label}</span>
                    <span className="text-[10px] opacity-70 font-semibold px-1 py-0.2 bg-indigo-200/60 dark:bg-indigo-900/80 rounded">
                      [{v.unit}]
                    </span>
                  </button>
                ))}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {COMMON_OPERATORS.map((op) => (
                    <button
                      ...
                      className="px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-mono font-bold transition-all active:scale-95 min-w-[40px] min-h-[38px] text-center cursor-pointer flex items-center justify-center"
                    >
                      {op.label}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {COMMON_CONSTANTS.map((c) => (
                    <button
                      ...
                      title={c.description}
                      className="px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-mono font-bold transition-all active:scale-95 min-h-[38px] cursor-pointer"
                    >
                      {c.label}
                    </button>
                  ))}
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold transition-all border border-slate-200 dark:border-slate-700 cursor-pointer min-h-[38px]"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-100 hover:bg-rose-50 dark:bg-slate-800 dark:hover:bg-rose-950/30 text-slate-600 hover:text-rose-600 dark:text-slate-300 dark:hover:text-rose-400 rounded-xl text-xs font-bold transition-all border border-slate-200 dark:border-slate-700 cursor-pointer min-h-[38px]"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-100 hover:bg-amber-50 dark:bg-slate-800 dark:hover:bg-amber-950/30 text-slate-600 hover:text-amber-600 dark:text-slate-300 dark:hover:text-amber-400 rounded-xl text-xs font-bold transition-all border border-slate-200 dark:border-slate-700 cursor-pointer min-h-[38px]"
              >
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-[11px] font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg transition-colors cursor-pointer"
            >
              {copiedLatex ? (
                <>
          ...
          <div className="bg-slate-50 dark:bg-slate-950/70 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-x-auto min-h-[64px] flex items-center justify-center text-center">
        ...
        <div className="bg-slate-50 dark:bg-slate-950/40 p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2.5 gap-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3 text-xs">
            {activeTab === "1PH" ? (
              <>
                ...
                <div className="p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs sm:col-span-2 lg:col-span-1">
                  <span className="text-slate-400 block font-bold text-[10px] uppercase">
          ...
          <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
            <div>
            ...
            <div className="w-full sm:w-auto flex items-center justify-end">
              ...
              currentTestEvaluation.result > 0 ? (
                <div className="w-full sm:w-auto px-5 py-2.5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/50 rounded-2xl text-center flex items-center justify-center sm:justify-end gap-2">
                ...
                <div className="w-full sm:w-auto px-4 py-2.5 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/50 rounded-2xl text-red-600 dark:text-red-400 text-xs font-bold text-center">
                  Calculation Error:{" "}
      ...
      <div className="shrink-0 p-4 sm:p-5 bg-slate-50/95 dark:bg-slate-950/95 border-t border-slate-200 dark:border-slate-800 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <button
          type="button"
          onClick={onClose}
          className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-all text-center cursor-pointer min-h-[42px] flex items-center justify-center"
        >
          Cancel
        </button>
        ...
        <div className="w-full sm:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          {saveSuccessNotification && (
            <span className="flex items-center justify-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 font-bold animate-pulse py-1">
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-500/20 active:scale-95 cursor-pointer min-h-[42px]"
          >
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-fade">
          <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 max-w-md w-full shadow-2xl space-y-4">
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-2">
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-center cursor-pointer min-h-[40px]"
              >
                Cancel
              </button>
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold transition-all shadow-md active:scale-95 text-center cursor-pointer min-h-[40px]"
              >
                Yes, Reset Formula
              </button>
            </div>
