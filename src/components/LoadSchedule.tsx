...
export default function LoadSchedule({
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 dark:bg-slate-950/20 p-4 rounded-xl border border-slate-100 dark:border-slate-850">
                  ...
                  <div className="bg-white dark:bg-zinc-950 p-2 rounded-xl border border-slate-200 dark:border-zinc-800 overflow-x-auto">
                    <LatexRenderer tex="\text{Max Demand Current (1}\Phi\text{)} = \left[ \left( \frac{\text{Total Connected VA}}{V_{\text{sys}}} \right) \times 0.80 + 0.25 \times \text{HML} \right]" />
                  </div>
                </div>
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl text-white">
                  ...
                  <div className="bg-zinc-950 p-4 rounded-xl overflow-x-auto min-h-[140px] flex items-center">
                    <div className="mx-auto">
                      <LatexRenderer
                        tex={`\\begin{aligned}
  I_{\\text{demand}} &= \\left[ \\left( \\frac{${(maxDemandDetails.internalConnectedVA || 0).toFixed(1)}}{${sysV}} \\right) \\times 0.80 + 0.25 \\times ${(maxDemandDetails.HML || 0).toFixed(2)} \\right] \\\\
  &= \\left[ \\left( ${((maxDemandDetails.internalConnectedVA || 0) / sysV).toFixed(3)} \\right) \\times 0.80 + ${(0.25 * (maxDemandDetails.HML || 0)).toFixed(3)} \\right] \\\\
  &= ${(((maxDemandDetails.internalConnectedVA || 0) / sysV) * 0.8 + 0.25 * (maxDemandDetails.HML || 0)).toFixed(2)} \\\\
  ...
  \\end{aligned}`}
                  ...
                  <div className="mt-4 flex justify-between items-center border-t border-zinc-800 pt-3">
                    ...
                    <button
                      onClick={() => {
                        const code = `\\text{Max Demand Current (1\\Phi)} = \\left[ \\left( \\frac{${(maxDemandDetails.internalConnectedVA || 0).toFixed(1)}}{${sysV}} \\right) \\times 0.80 + 0.25 \\times ${(maxDemandDetails.HML || 0).toFixed(2)} \\right] = ${mainCurrent.designAmp.toFixed(2)}\\text{ A}`;
