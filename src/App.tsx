...
import LoadSchedule, { INITIAL_CIRCUITS, INITIAL_PANEL } from './components/LoadSchedule';
...
import { STANDARD_CB_RATINGS, WIRE_AMPACITY_TABLE } from './constants';
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'schedule' | 'isc' | 'vd' | 'lighting'>('schedule');
  const [panel, setPanel] = useState<PanelConfig>(INITIAL_PANEL);
  const [circuits, setCircuits] = useState<Circuit[]>(INITIAL_CIRCUITS);
  const [subPanels, setSubPanels] = useState<{ id: string, panel: PanelConfig, circuits: Circuit[] }[]>([]);

  const tabs = [
    { id: 'schedule', label: 'Load Schedule', icon: Layout, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { id: 'isc', label: 'Short Circuit', icon: ShieldAlert, color: 'text-red-600', bg: 'bg-red-50' },
    { id: 'vd', label: 'Voltage Drop', icon: Ruler, color: 'text-green-600', bg: 'bg-green-50' },
    { id: 'lighting', label: 'Illumination', icon: Lightbulb, color: 'text-yellow-500', bg: 'bg-yellow-50' },
  ];
  ...
  const handlePrint = async () => {
    ...
    try {
      // Temporarily hide no-print elements and fix layout for PDF export
      ...
      const elementsToShow = document.querySelectorAll('.print-show');
      elementsToShow.forEach((el) => {
        el.classList.remove('hidden');
        el.classList.add('block', 'temp-print-show');
      });

      const overflowElements = document.querySelectorAll('.overflow-x-auto');
      overflowElements.forEach((el) => {
        el.classList.remove('overflow-x-auto');
        el.classList.add('overflow-visible');
        el.classList.add('temp-overflow-fix');
      });
      ...
      // Restore elements
      const restoredShows = document.querySelectorAll('.temp-print-show');
      restoredShows.forEach((el) => {
        el.classList.add('hidden');
        el.classList.remove('block');
        el.classList.remove('temp-print-show');
      });

      const restoredOverflows = document.querySelectorAll('.temp-overflow-fix');
      restoredOverflows.forEach((el) => {
        el.classList.add('overflow-x-auto');
        el.classList.remove('overflow-visible');
        el.classList.remove('temp-overflow-fix');
      });
      ...
      const imgWidth = img.width / 2; // adjust for pixelRatio
      ...
      const outputWidth = availableWidth;
      const outputHeight = imgHeight * ratio;
      
      let remainingHeight = outputHeight;
      let yOffset = 0;
      ...
      while (remainingHeight > 0) {
        ...
        pdf.addImage(dataUrl, 'PNG', margin, margin - yOffset, outputWidth, outputHeight);
        
        remainingHeight -= pageHeight;
        yOffset += pageHeight;
      }

      const filename = `${tabs.find(t => t.id === activeTab)?.label || 'Export'}.pdf`;
  ...
  const exportToExcel = () => {
    ...
    allPanelsToExport.forEach((item, index) => {
      ...
      const wireAmpacity = WIRE_AMPACITY_TABLE.find(w => w.size === wireSizeForGnd)?.ampacity || 20;
      ...
      const phaseImbalance = maxPhaseLoad > 0 ? (1 - (Math.min(loads.R, loads.Y, loads.B) / maxPhaseLoad)) * 100 : 0;

      const wsData: any[][] = [];
      wsData.push(['PROJECT:', p.project, '', 'SYSTEM:', p.system]);
      wsData.push(['PANEL DESIGNATION:', p.designation, '', 'VOLTAGE:', p.voltage]);
      wsData.push([]);
      ...
      if (p.system.includes('3PH')) {
        wsData.push(['', '', '', '', '', '', 'AB', 'BC', 'CA', '', '', '', '', '', '']);
      }

      c.forEach(cir => {
        const row: any[] = [
          cir.circuitNo, cir.description, cir.wattage, cir.quantity, cir.loadVA, cir.phase,
        ];
        ...
        row.push(
          cir.mcbAT, cir.mcbAF, cir.mcbP, cir.mcbKAIC, cir.mcbType,
      ...
      wsData.push([]);
      const baseTotalRow: any[] = ['', '', '', 'Total Connected Load', `${totalVA.toFixed(0)} VA`, ''];
      if (is3Phase) {
        ...
        baseTotalRow.push(`${amps.R.toFixed(2)} A`, `${amps.Y.toFixed(2)} A`, `${amps.B.toFixed(2)} A`);
      } else {
        baseTotalRow.push(`${mainCurrent.toFixed(2)} A`);
      }
      wsData.push(baseTotalRow);
      wsData.push(['', '', '', 'Total kVA', `${(totalVA/1000).toFixed(2)} kVA`]);
      
      const cbAF = cb <= 50 ? 50 : cb <= 100 ? 100 : cb <= 225 ? 225 : cb <= 400 ? 400 : 600;
      const poles = is3Phase ? 3 : 2;
      
      const branchTypeCounts = c.reduce((acc, cir) => {
      ...
      if (cb > 100 && (type === 'Plug-in' || type === 'Bolt-on' || type === 'MCB')) {
        type = 'MCCB';
      }
      ...
      wsData.push(['Main Feeder:', `${formatWireSize(wire.size)}mm² THHN, ${groundSize}mm² GND in ${conduitSize} PVC`]);
      wsData.push(['Main Breaker:', `${cb}A AT / ${cbAF}AF, ${poles}P, ${kaic}kAIC, ${type}`]);
      ...
      for (let R = range.s.r; R <= range.e.r; ++R) {
        for (let C = range.s.c; C <= range.e.c; ++C) {
          ...
          } else if (R >= 4 + headerRowOffset && R < 4 + headerRowOffset + c.length) { 
  ...
  const handleExportWord = async () => {
    try {
      const getImg = async (id: string) => {
        ...
        if (!el) return null;
        return await toPng(el, { quality: 1, backgroundColor: '#ffffff', pixelRatio: 2 });
      };
      ...
      const images = {
        ...
        vd: await getImg('voltage-drop-diagram'),
        illumination: await getImg('illumination-diagram')
      };

      await exportToWord(panel, circuits, subPanels, images);
    } catch (e) {
      console.error('Error generating Word doc:', e);
      alert('There was an issue generating the Word document. Check console for details.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center">
        ...
        <div className="max-w-[1600px] mx-auto px-4 flex items-center h-16 justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:block">
              <span className="font-black text-slate-900 tracking-tight text-lg">PEC PRO</span>
              <p className="text-[10px] text-slate-400 font-bold uppercase -mt-1">PH Engineering Tool</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={handlePrint}
              className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition-colors shadow-lg shadow-slate-200"
            >
              <Printer className="w-4 h-4" />
              <span className="hidden lg:inline">Print / Export PDF</span>
            </button>
          </div>
        </div>
      </nav>
      ...
      {/* Main Content Area */}
      <main id="print-area" className="w-full max-w-[1600px] p-4 md:p-8 flex flex-col items-center gap-8">
          ...
          >
            {activeTab === 'schedule' && (
              <div className="flex flex-col gap-12 w-full max-w-full">
                <LoadSchedule panel={panel} setPanel={setPanel} circuits={circuits} setCircuits={setCircuits} availableSubPanels={subPanels} />
                ...
                {subPanels.map((sp, index) => (
                  <React.Fragment key={sp.id}>
                    <LoadSchedule 
                      panel={sp.panel} 
                      setPanel={(newPanel) => {
                        setSubPanels(prev => prev.map((p, i) => i === index ? { ...p, panel: typeof newPanel === 'function' ? newPanel(p.panel) : newPanel } : p));
                      }}
                      circuits={sp.circuits} 
                      setCircuits={(newCircuits) => {
                        setSubPanels(prev => prev.map((p, i) => i === index ? { ...p, circuits: typeof newCircuits === 'function' ? newCircuits(p.circuits) : newCircuits } : p));
                      }}
                      isSubPanel={true}
                      onRemoveSubPanel={() => {
                  ...
                  onClick={() => {
                    setSubPanels(prev => [
                      ...
                      { 
                        ...
                        panel: { ...INITIAL_PANEL, designation: `Sub-Panel ${prev.length + 1}` }, 
            ...
            {activeTab === 'isc' && <ShortCircuitCalc panel={panel} circuits={circuits} />}
            {activeTab === 'vd' && <VoltageDropCalc panel={panel} circuits={circuits} />}
            {activeTab === 'lighting' && <IlluminationCalc circuits={circuits} setCircuits={setCircuits} setActiveTab={setActiveTab} />}
          </motion.div>
        </AnimatePresence>
      </main>
      ...
      <div className="fixed top-0 left-[-9999px] w-[1000px] opacity-0 pointer-events-none flex flex-col gap-8 no-print z-[-10] bg-slate-50 min-h-screen">
         ...
         <div id="export-container-isc">
            <ShortCircuitCalc panel={panel} circuits={circuits} />
         </div>
         <div id="export-container-vd">
            <VoltageDropCalc panel={panel} circuits={circuits} />
         </div>
         <div id="export-container-illum">
            <IlluminationCalc circuits={circuits} setCircuits={setCircuits} setActiveTab={setActiveTab} />
         </div>
      </div>
         ...
         <div className="max-w-[1600px] mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex flex-col items-center md:items-start">
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-bold text-slate-900">PEC Load Schedule Pro</span>
               </div>
               <p className="text-xs text-slate-400 max-w-xs text-center md:text-left">
                  Professional calculators based on PEC Part 1 and Part 2. High-fidelity design for electrical engineers and contractors.
               </p>
            </div>
            <div className="flex gap-8">
               <div className="flex flex-col gap-2">
                  <span className="text-[10px] font-black text-slate-300 uppercase letter tracking-widest text-center md:text-left">Standards</span>
                  <div className="flex gap-4 opacity-30 grayscale items-center h-8">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/b/ba/IEEE_Logo.svg" className="h-4" alt="IEEE" />
                    <span className="text-xs font-bold text-slate-900">PEC 2017</span>
                  </div>
               </div>
            </div>
         </div>
