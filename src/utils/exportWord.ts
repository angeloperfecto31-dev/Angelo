...
Using PEC rules, the Maximum Demand Current is calculated as:`;
      ...
      formulaText = `I_{\\text{demand}} = \\left[ \\left( \\frac{\\text{Total Connected VA}}{V_{sys}} \\times 0.80 \\right) + \\left( 0.25 \\times \\text{HML} \\right) \\right] \\times 1.25 = \\left[ \\left( \\frac{${(maxDemandDetails.internalConnectedVA || 0).toFixed(1)}}{${sysVWord}} \\times 0.80 \\right) + \\left( 0.25 \\times ${(maxDemandDetails.HML || 0).toFixed(2)} \\right) \\right] \\times 1.25 = ${((((maxDemandDetails.internalConnectedVA || 0) / sysVWord) * 0.8 + 0.25 * (maxDemandDetails.HML || 0)) * 1.25).toFixed(2)}\\text{ A}`;
    }
    ...
    docChildren.push(
      ...
      createParagraph(`• Transformer Connection: ${p.transformerConnection || 'Delta-Wye (Δ-Y)'}`),
