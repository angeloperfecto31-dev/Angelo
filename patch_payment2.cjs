...
code = code.replace(/Basic Tier/g, "Basic (Paid)");
code = code.replace(/!u\.isActive \? \(/g, '!u.isActive && u.paymentStatus === "unpaid" ? ('); // if that exists
...
// Try replacing it regardless of whitespace
code = code.replace(/\{u\.plan === "enterprise" \? \([\s\S]*?Basic \(Paid\)[\s\S]*?<\/span>\n\s*\)\}/, replacement);

fs.writeFileSync('src/components/PaymentScreen.tsx', code);
console.log('PaymentScreen.tsx patched 2');
